...
export default function ChatScreen({ session, messages, setMessages, onFinished, onRestart }) {
  ...
  // Countdown for the current question. Driven off an absolute deadline and the
  // wall clock so a backgrounded (throttled) tab doesn't let it drift — on
  // refocus it snaps to the right value.
  useEffect(() => {
    if (!awaitingAnswer || !currentLimit) {
      setSecondsLeft(null);
      return;
    }
    const deadline = Date.now() + currentLimit * 1000;
  ...
  }, [lastMsgId, awaitingAnswer, currentLimit]);
